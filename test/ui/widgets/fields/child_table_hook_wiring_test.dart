...
void main() {
  ...
  Future<void> pumpFactoryField(
    ...
    await tester.pumpWidget(
      MaterialApp(home: Scaffold(body: built!)),
    );
    await tester.pump();
  }

  testWidgets('H3 — resolveLinkTitle set on the factory reaches ChildTableField', (
    tester,
  ) async {
    ...
    expect(
      ...
      same(resolver),
      reason: 'the hook must arrive by the SAME instance, not be reconstructed',
    );
  });

  testWidgets('H3 — rowNoticeBuilder set on the factory reaches ChildTableField', (
    tester,
  ) async {
    String? notice(
      String childDoctype,
...
void _h4AndM2bTests() {
  ...
  testWidgets('H4 — a parent rebuild that does not touch the rows re-resolves nothing', (
    tester,
  ) async {
    ...
    expect(
      afterFirstPaint,
      greaterThan(0),
      reason: 'the rows must resolve at least once, or the test proves nothing',
    );
  ...
  testWidgets('M2b — a read-only child grid wraps its sheet body in AbsorbPointer', (
    tester,
  ) async {
